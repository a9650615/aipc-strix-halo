#!/usr/bin/env python3
"""Release Lemonade compact aliases after configured idle periods."""
...
def _load_manifest(path: Path) -> list[dict]:
    ...
    for entry in models:
        ...
        if not isinstance(idle, (int, float)):
            name = entry.get("alias", entry.get("model_id", "<unknown>"))
        ...
        if model_id:
            result.append({
                ...
                "model_id": model_id,
                "idle_unload_after_s": float(idle),
            })
    return result
...
def run(base_url: str, manifest_path: Path) -> int:
    ...
    if not manifest:
        print("lemonade-idle-release: no idle policies declared", file=sys.stderr)
        return 0
...
def self_test() -> None:
    ...
    manifest = [
        ...
        {"alias": "ornith-35b", "model_id": "Ornith-1.0-35B-GGUF-Q4_K_M"},
    ]
