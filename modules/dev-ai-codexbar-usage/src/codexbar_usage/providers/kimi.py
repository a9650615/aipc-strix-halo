...
- Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift
- Sources/CodexBarCore/Providers/Kimi/KimiBalanceAPI.swift

References:
- API: https://api.moonshot.cn/v1/balance
- Auth: Bearer token (Moonshot/Kimi API key, prefix: sk-...)
- Linux Support: full
"""
...
# Moonshot/Kimi API base URL.
KIMI_API_BASE = "https://api.moonshot.cn/v1"
# Balance endpoint.
KIMI_BALANCE_PATH = "/balance"


class KimiProvider(BaseProvider):
    """Kimi provider using Moonshot balance API.

    Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift

    Kimi (by Moonshot AI) uses the same API infrastructure as Moonshot.
    The balance endpoint provides account credit information.

    Supports:
    - Bearer token authentication (Moonshot/Kimi API key)
    ...
    def __init__(
        ...
        base_url: str = KIMI_API_BASE,
        config: Optional[Dict[str, Any]] = None,
    ):
        """Initialize Kimi provider.

        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:init

        Args:
            api_key: Kimi / Moonshot API key (Bearer token, prefix: sk-...).
        ...
        self._logger = logging.getLogger(f"providers.kimi")

    async def fetch_usage(self) -> UsageSnapshot:
        """Fetch Kimi account balance.

        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:fetchUsage()
        ...
        try:
            ...
            self.last_error = ProviderError(f"Failed to fetch Kimi balance: {e}")
            raise self.last_error

    async def _fetch_balance(self) -> Dict[str, Any]:
        """Fetch balance from Moonshot/Kimi API.

        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiBalanceAPI.swift:fetchBalance()
        ...
        """
        url = self._validate_endpoint(KIMI_BALANCE_PATH)
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 401:
                    raise AuthError("Invalid Kimi / Moonshot API key")
                if response.status_code == 403:
                    raise AuthError("Kimi API key lacks balance access")
    ...
    def _parse_balance(self, data: Dict[str, Any]) -> Optional[RateWindow]:
        ...
        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:_parseBalance()

        Kimi uses a prepaid credit system. The rate window represents
        ...
        try:
            ...
            # Moonshot/Kimi balance API may return different field names.
            ...
            if total_credit > 0:
                ...
                # Fallback: treat balance as remaining, assume some total.
                used_percent = 0.0
            else:
                used_percent = 0.0
            ...
            return RateWindow(
                ...
                window_minutes=None,
                resets_at=None,
                reset_description="Kimi prepaid credits, no fixed reset",
            )

        except Exception as e:
            logger.warning(f"Failed to parse Kimi balance: {e}")
            return None

    def _parse_identity(self, data: Dict[str, Any]) -> Optional[ProviderIdentitySnapshot]:
        """Parse identity from balance data.

        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:_parseIdentity()
        ...
        try:
            ...
            logger.warning(f"Failed to parse Kimi identity: {e}")
            return None

    def get_api_key_from_env(self) -> Optional[str]:
        """Get API key from environment variables.

        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:getApiKeyFromEnv()

        Checks:
        1. KIMI_API_KEY (primary)
        2. MOONSHOT_API_KEY (fallback, Kimi uses Moonshot infrastructure)
    ...
    def get_provider_metadata() -> Dict[str, Any]:
        ...
        Ported from: Sources/CodexBarCore/Providers/Kimi/KimiProvider.swift:getProviderMetadata()
        ...
        return {
            "provider_id": "kimi",
            "name": "Kimi",
            "description": "Kimi (Moonshot AI) Balance",
            "linux_support": "full",
            "auth_method": "bearer_token",
            "config_env_vars": ["KIMI_API_KEY", "MOONSHOT_API_KEY"],
            "dashboard_url": "https://kimi.moonshot.cn/settings/keys",
