...
- [ ] 2.1 Add five cloud model entries to `modules/llm-litellm/files/etc/aipc/litellm/config.yaml`:
...
- [ ] 3.1 Add five entries to `modules/llm-models/files/etc/aipc/models/models.yaml`:
  ```yaml
  - alias: main-cloud
    backend: cloud
...
- [ ] 7.4 Smoke-test: `curl http://127.0.0.1:4000/v1/chat/completions -d '{"model":"main-cloud","messages":[...]}'` — confirm response from Anthropic.
